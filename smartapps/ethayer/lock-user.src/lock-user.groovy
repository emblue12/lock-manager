...
def setupPage() {
  dynamicPage(name: 'setupPage', title: 'Setup Lock', nextPage: 'mainPage', uninstall: true) {
    section('Choose devices for this lock') {
      input(name: 'userName', title: 'Name for User', required: true)
...
def mainPage() {
  ...
  dynamicPage(name: 'mainPage', title: '', install: true, uninstall: true) {
    ...
    section('Additional Settings') {
      ...
      if (actions) {
        actions.sort()
        input name: 'userUnlockPhrase', type: 'enum', title: 'Hello Home Phrase on unlock', multiple: true, required: false, options: actions, refreshAfterSelection: true
        input name: 'userLockPhrase', type: 'enum', title: 'Hello Home Phrase on lock', description: 'Available on select locks only', multiple: true, required: false, options: actions, refreshAfterSelection: true
    ...
    section('Setup', hideable: true, hidden: true) {
      label(title: "Name for App", defaultValue: 'User: ' + userName, required: true)
      input name: 'userName', title: "Name for user", required: true
      input(name: "userSlot", type: "enum", options: parent.availableSlots(settings.userSlot), title: "Select slot", required: true, refreshAfterSelection: true )
  ...
  dynamicPage(name:"lockPage", title:"Lock Settings") {
    ...
    if (!state."lock${lock.id}".enabled) {
      section {
        ...
        href(name: 'toReEnableUserLockPage', page: 'reEnableUserLockPage', title: 'Reset User', description: 'Retry setting this user.',  params: [id: lock.id])
      }
    }
  ...
  dynamicPage(name: 'notificationPage', title: 'Notification Settings') {
    ...
    section {
      if (phone == null && !notification && !recipients) {
        input(name: 'muteUser', title: 'Mute this user?', type: 'bool', required: false, submitOnChange: true, defaultValue: false, description: 'Mute notifications for this user if notifications are set globally')
      }
      if (!muteUser) {
